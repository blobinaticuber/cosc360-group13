...
import { AdminCredentialsSchema, AdminDetailsSchema, AdminPersonalDetailsSchema, AdminRegistrationSchema, AdminUserUpdateSchema } from "./adminUser.js"
import { ErrConflictSchema, ErrInvalidBodySchema } from "util/errSchema.js"
import z from "zod"

const adminUserSpec = new OpenAPIRegistry()

adminUserSpec.registerPath({
	method: "post",
	path: "/admin/user",
...
adminUserSpec.registerPath({
	...
	responses: {
		200: {
			description: "The admin user is now logged in (and a cookie is set)."
		},
		404: {
...
adminUserSpec.registerPath({
	...
	description: "Retrieves the personal data about the currently logged-in admin user.",
	request: {
		cookies: z.object({
			[process.env.AUTH_COOKIE!]: z.string().meta({
				description: "The authentication cookie for an admin user session. One of these will be set on a client after a successful admin login."
	...
	responses: {
		...
		404: {
			description: "The admin user's session was identified, but their details couldn't be retrieved. This is extremely unlikely to ever happen."
		}
	}
})
...
adminUserSpec.registerPath({
	...
	description: "Updates the details about the currently logged-in admin user.",
	request: {
		cookies: z.object({
			[process.env.AUTH_COOKIE!]: z.string().meta({
				description: "The authentication cookie for an admin user session. One of these will be set on a client after a successful admin login."
